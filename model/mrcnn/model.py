...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2), use_bias=True, train_bn=True):
    ...
    x = KL.Conv2D(
        nb_filter2, (kernel_size, kernel_size), padding="same", name=conv_name_base + "2b", use_bias=use_bias
    )(x)
    x = BatchNorm(name=bn_name_base + "2b")(x, training=train_bn)
    x = KL.Activation("relu")(x)
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        pre_nms_anchors = utils.batch_slice(
            [anchors, ix], lambda a, x: tf.gather(a, x), self.config.IMAGES_PER_GPU, names=["pre_nms_anchors"]
        )
        ...
        boxes = utils.batch_slice(
            boxes, lambda x: clip_boxes_graph(x, window), self.config.IMAGES_PER_GPU, names=["refined_anchors_clipped"]
        )
        ...
        def nms(boxes, scores):
            indices = tf.image.non_max_suppression(
                boxes, scores, self.proposal_count, self.nms_threshold, name="rpn_non_max_suppression"
            )
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = K.switch(
        tf.size(input=target_bbox) > 0, smooth_l1_loss(y_true=target_bbox, y_pred=pred_bbox), tf.constant(0.0)
    )
    loss = K.mean(loss)
    return loss
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    masks = np.zeros(
        (config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0], config.MASK_SHAPE[1], config.NUM_CLASSES), dtype=np.float32
    )
    for i in pos_ids:
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        rpn_rois = ProposalLayer(
            proposal_count=proposal_count, nms_threshold=config.RPN_NMS_THRESHOLD, name="ROI", config=config
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            if layer.output in self.keras_model.losses:
                continue
            loss = tf.reduce_mean(input_tensor=layer.output, keepdims=True) * self.config.LOSS_WEIGHTS.get(name, 1.0)
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            image_meta = compose_image_meta(
                0, image.shape, molded_image.shape, window, scale, np.zeros([self.config.NUM_CLASSES], dtype=np.int32)
            )
            # Append
