...
def identity_block(
    ...
    x = KL.Conv2D(
        nb_filter1, (1, 1), name=conv_name_base + "2a", use_bias=use_bias
    )(input_tensor)
    ...
    x = KL.Conv2D(
        nb_filter3, (1, 1), name=conv_name_base + "2c", use_bias=use_bias
    )(x)
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        ix = tf.nn.top_k(
            scores, pre_nms_limit, sorted=True, name="top_anchors"
        ).indices
        ...
        def nms(boxes, scores):
            ...
            proposals = tf.pad(
                tensor=proposals, paddings=[(0, padding), (0, 0)]
            )
            return proposals
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        box_to_level = tf.concat(
            [tf.cast(box_to_level, tf.int32), box_range], axis=1
        )
        ...
        ix = tf.nn.top_k(
            sorting_tensor, k=tf.shape(input=box_to_level)[0]
        ).indices[::-1]
...
def overlaps_graph(boxes1, boxes2):
    ...
    b1 = tf.reshape(
        tf.tile(tf.expand_dims(boxes1, 1), [1, 1, tf.shape(input=boxes2)[0]]),
        [-1, 4],
    )
...
def detection_targets_graph(
    proposals, gt_class_ids, gt_boxes, gt_masks, config
):
    ...
    negative_count = (
        tf.cast(r * tf.cast(positive_count, tf.float32), tf.int32)
        - positive_count
    )
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    detections = tf.pad(
        tensor=detections, paddings=[(0, gap), (0, 0)], mode="CONSTANT"
    )
    return detections
...
def fpn_classifier_graph(
    ...
    shared = KL.Lambda(
        lambda x: K.squeeze(K.squeeze(x, 3), 2), name="pool_squeeze"
    )(x)
...
def build_fpn_mask_graph(
    ...
    x = KL.TimeDistributed(BatchNorm(), name="mrcnn_mask_bn1")(
        x, training=train_bn
    )
    x = KL.Activation("relu")(x)
    ...
    x = KL.TimeDistributed(BatchNorm(), name="mrcnn_mask_bn2")(
        x, training=train_bn
    )
    x = KL.Activation("relu")(x)
    ...
    x = KL.TimeDistributed(BatchNorm(), name="mrcnn_mask_bn3")(
        x, training=train_bn
    )
    x = KL.Activation("relu")(x)
    ...
    x = KL.TimeDistributed(BatchNorm(), name="mrcnn_mask_bn4")(
        x, training=train_bn
    )
    x = KL.Activation("relu")(x)
...
def mrcnn_class_loss_graph(
    target_class_ids, pred_class_logits, active_class_ids
):
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = KL.Lambda(lambda x: norm_boxes_graph2(x))(
...
def trim_zeros_graph(boxes, name="trim_zeros"):
    ...
    """
    non_zeros = tf.cast(
        tf.reduce_sum(input_tensor=tf.abs(boxes), axis=1), tf.bool
    )
...
def evaluate_model(dataset: utils.Dataset, model: KM.Model, cfg: Config):
    """Evaluates the trained model on the given dataset.

    Args:
        dataset (utils.Dataset): test dataset
        model (KM.Model): keras model to evaluate
